...
- `data/topic_library.csv`：底层选题库，只写 PRD 指定前 10 个字段；创建时间统一为 `YYYY-MM-DD`。
...
- 可以用 `library format` 把 CSV 的创建时间归一为 `YYYY-MM-DD`，并生成带列宽、自动换行和勾选列的 `data/topic_library.xlsx`；这是导出辅助，不再是主确认流程。
